...
import { AlertCircle, CheckCircle, Users, UserPlus, UserMinus, GraduationCap, BookOpen } from "lucide-react";
...
function CourseTakingPage() {
  ...
  // 学生管理
  const [takingStudents, setTakingStudents] = useState<TakingData>({ registered: [], unregistered: [] });
  const [selectedUnregisteredStudents, setSelectedUnregisteredStudents] = useState<number[]>([]);
  const [selectedRegisteredStudents, setSelectedRegisteredStudents] = useState<number[]>([]);

  // 教師管理
  const [grantTeachers, setGrantTeachers] = useState<GrantData>({ registered: [], unregistered: [] });
  const [selectedUnregisteredTeachers, setSelectedUnregisteredTeachers] = useState<number[]>([]);
  const [selectedRegisteredTeachers, setSelectedRegisteredTeachers] = useState<number[]>([]);

  const [activeTab, setActiveTab] = useState<"add_students" | "conf_students" | "add_teachers" | "conf_teachers">("add_students");
  ...
  const fetchTakingStudents = async () => {
    try {
      const response = await axios.get(`/get_taking_students/${params.course_id}`);
  ...
  const fetchGrantTeachers = async () => {
    try {
      const response = await axios.get(`/get_grant_teachers/${params.course_id}`);
          ...
          <div className="flex flex-col space-y-4 mb-6">
            ...
            <div>
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  variant={activeTab === "conf_students" ? "default" : "outline"}
            ...
            <div>
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  variant={activeTab === "conf_teachers" ? "default" : "outline"}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {courseInfo?.update_answer && (
                      <Button
                        ...
                        disabled={selectedUnregisteredStudents.length === 0 || loading}
                        className="flex items-center gap-2"
                      >
                        <UserPlus className="h-4 w-4" />
                        {loading ? "追加中..." : "追加"}
                  ...
                  <div className="space-y-4">
                    {courseInfo?.update_answer && (
                      <div className="flex items-center gap-2">
                        <Checkbox
                          checked={selectedUnregisteredStudents.length === takingStudents.unregistered.length && takingStudents.unregistered.length > 0}
                      ...
                      {takingStudents.unregistered.length === 0 ? (
                        ...
                        takingStudents.unregistered.map((student) => (
                          <div key={student.id} className="flex items-center space-x-3 p-3 border rounded-lg">
                            {courseInfo?.update_answer && (
                              <Checkbox
                                checked={selectedUnregisteredStudents.includes(student.id)}
                                onCheckedChange={() => handleStudentSelection(student.id, false)}
                              />
                            )}
                            ...
                            <Badge variant="secondary">{student.kind_name}</Badge>
                          </div>
                        ))
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {courseInfo?.update_answer && (
                      <Button
                        ...
                        disabled={selectedRegisteredStudents.length === 0 || loading}
                        variant="destructive"
                        className="flex items-center gap-2"
                      >
                ...
                <CardContent>
                  <div className="space-y-4">
                    {courseInfo?.update_answer && (
                      <div className="flex items-center gap-2">
                        <Checkbox
                          checked={selectedRegisteredStudents.length === takingStudents.registered.length && takingStudents.registered.length > 0}
                      ...
                      {takingStudents.registered.length === 0 ? (
                        ...
                        takingStudents.registered.map((student) => (
                          <div key={student.id} className="flex items-center space-x-3 p-3 border rounded-lg">
                            {courseInfo?.update_answer && (
                              <Checkbox
                                checked={selectedRegisteredStudents.includes(student.id)}
                                onCheckedChange={() => handleStudentSelection(student.id, true)}
                              />
                            )}
                            <div className="flex-1">
                              <div className="font-medium">{student.email}</div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {courseInfo?.update_answer && (
                      <Button
                        ...
                        disabled={selectedUnregisteredTeachers.length === 0 || loading}
                        className="flex items-center gap-2"
                      >
                        <UserPlus className="h-4 w-4" />
                        {loading ? "付与中..." : "付与"}
                ...
                <CardContent>
                  <div className="space-y-4">
                    {courseInfo?.update_answer && (
                      <div className="flex items-center gap-2">
                        <Checkbox
                          checked={selectedUnregisteredTeachers.length === grantTeachers.unregistered.length && grantTeachers.unregistered.length > 0}
                      ...
                      {grantTeachers.unregistered.length === 0 ? (
                        ...
                        grantTeachers.unregistered.map((teacher) => (
                          <div key={teacher.id} className="flex items-center space-x-3 p-3 border rounded-lg">
                            {courseInfo?.update_answer && (
                              <Checkbox
                                checked={selectedUnregisteredTeachers.includes(teacher.id)}
                                onCheckedChange={() => handleTeacherSelection(teacher.id, false)}
                              />
                            )}
                            <div className="flex-1">
                            ...
                            <Badge variant="secondary">{teacher.kind_name}</Badge>
                          </div>
                        ))
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {courseInfo?.update_answer && (
                      <Button
                        ...
                        disabled={selectedRegisteredTeachers.length === 0 || loading}
                        variant="destructive"
                        className="flex items-center gap-2"
                      >
                        <UserMinus className="h-4 w-4" />
                ...
                <CardContent>
                  <div className="space-y-4">
                    {courseInfo?.update_answer && (
                      <div className="flex items-center gap-2">
                        <Checkbox
                          checked={selectedRegisteredTeachers.length === grantTeachers.registered.length && grantTeachers.registered.length > 0}
                      ...
                      {grantTeachers.registered.length === 0 ? (
                        ...
                        grantTeachers.registered.map((teacher) => (
                          <div key={teacher.id} className="flex items-center space-x-3 p-3 border rounded-lg">
                            {courseInfo?.update_answer && (
                              <Checkbox
                                checked={selectedRegisteredTeachers.includes(teacher.id)}
                                onCheckedChange={() => handleTeacherSelection(teacher.id, true)}
                              />
                            )}
                            <div className="flex-1">
                              <div className="font-medium">{teacher.email}</div>
