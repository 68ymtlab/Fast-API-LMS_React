...
import { AlertCircle, BarChart3, Filter, Download, TrendingUp } from "lucide-react";
...
function CourseScorePage() {
  ...
  const fetchScoreData = async () => {
    try {
      setLoading(true);
      setErrorMessage("");
      const response = await axios.get(`/get_flow_session_teacher_score/${params.course_id}`);
  ...
  const extractGradesByExercise = (data: ContentData[]): { [exerciseName: string]: number[] } => {
  ...
  const chartData = useMemo(() => {
    ...
    const filteredDatasets = selectedExercises.length > 0 
      ? datasets.filter(dataset => selectedExercises.includes(dataset.label))
      : datasets;
                ...
                <Card className="bg-blue-50 border-blue-200">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-sm font-medium text-blue-800">選択中の演習問題:</span>
                      ...
                      <table className="w-full border-collapse border border-gray-300">
                        <thead>
                          <tr className="bg-blue-600 text-white">
                            <th className="border border-gray-300 p-3 text-left font-bold">演習問題</th>
                            <th className="border border-gray-300 p-3 text-center font-bold">平均値</th>
                            <th className="border border-gray-300 p-3 text-center font-bold">最大値</th>
                            <th className="border border-gray-300 p-3 text-center font-bold">最小値</th>
                            <th className="border border-gray-300 p-3 text-center font-bold">中央値</th>
                            <th className="border border-gray-300 p-3 text-center font-bold">データ数</th>
                          </tr>
                        </thead>
                        <tbody>
                          {Object.entries(exerciseStatistics).map(([exerciseName, stats], index) => (
                            <tr
                              key={exerciseName}
                              className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-gray-100 transition-colors`}
                            >
                              <td className="border border-gray-300 p-3 font-medium">{exerciseName}</td>
                              ...
                              <td className="border border-gray-300 p-3 text-center">
                                {stats.count > 0 ? `${stats.median.toFixed(1)}%` : '-'}
                              </td>
                              <td className="border border-gray-300 p-3 text-center">
                                {stats.count}
                              </td>
              ...
              {exerciseList.map(exercise => (
                <div key={exercise} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => handleExerciseSelection(exercise, checked as boolean)}
                  />
