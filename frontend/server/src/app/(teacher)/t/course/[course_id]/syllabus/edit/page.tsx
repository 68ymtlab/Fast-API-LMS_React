...
import { AlertCircle, CheckCircle, BookOpen, Edit, Save, Award, Target, Hash, Calendar } from "lucide-react";
...
const formSchema = z.object({
  ...
  subject_credit: z.coerce.number().min(1, "単位数は1以上で入力してください").max(10, "単位数は10以下で入力してください"),
...
function EditSyllabusPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const response = await axios.post(`/update_syllabus_info/${params.course_id}`, data);
      ...
      if (response.data.success) {
        ...
        setErrorMessage(response.data.error_msg || "シラバス情報の更新に失敗しました");
      }
    } catch (error: any) {
                        ...
                        <Select onValueChange={field.onChange} value={field.value} disabled={loading}>
                          ...
                          <SelectContent>
                            {subjectClasses.map((subjectClass) => (
                              <SelectItem key={subjectClass} value={subjectClass}>
                                {subjectClass}
                              </SelectItem>
                            ))}
                          </SelectContent>
