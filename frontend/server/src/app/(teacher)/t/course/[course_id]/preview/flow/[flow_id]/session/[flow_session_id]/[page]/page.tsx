...
function FlowSessionPreviewPage() {
	...
	const fetchSessionData = async () => {
		try {
			...
			const response = await axios.get(
				`/get_flow_session_info/${params.flow_session_id}?page=${currentPage}`,
			);
	...
	const goToPage = (page: number) => {
		router.push(
			`/t/course/${params.course_id}/preview/flow/${params.flow_id}/session/${params.flow_session_id}/${page}`,
		);
	};
					...
					{sessionInfo && (
						<div className="space-y-8">
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<div className="flex items-center gap-2 mt-2">
												...
												<div
													className={`flex items-center gap-1 px-2 py-1 rounded text-xs text-white ${getSessionTypeColor(sessionInfo.session_type)}`}
												>
													{getSessionTypeIcon(sessionInfo.session_type)}
										...
										<div className="flex items-center gap-2">
											<Link
												href={`/t/course/${params.course_id}/week/${sessionInfo.flow_id}/edit`}
											>
								...
								<Card>
									<CardContent className="p-4">
										<div className="flex items-center justify-between">
											...
											<div className="flex items-center gap-2">
												{Array.from(
													{ length: sessionInfo.total_pages },
													(_, i) => i + 1,
												).map((page) => (
													<Button
														key={page}
										...
										<div className="flex items-center gap-2">
											{currentPage < sessionInfo.total_pages ? (
												...
												<Button
													...
													onClick={() =>
														router.push(
															`/t/course/${params.course_id}/preview/flow/${params.flow_id}`,
														)
													}
													className="flex items-center gap-2"
												>
							...
							<div className="flex justify-center gap-4">
								<Link
									href={`/course/${params.course_id}/flow/${params.flow_id}/session/${params.flow_session_id}/1`}
								>
