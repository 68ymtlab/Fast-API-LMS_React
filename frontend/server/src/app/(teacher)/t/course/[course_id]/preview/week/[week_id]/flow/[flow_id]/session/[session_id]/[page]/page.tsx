...
function TeacherFlowSessionPreviewPage() {
	...
	const fetchQuestionData = async () => {
		try {
			...
			const response = await axios.get(
				`/get_flowpage/${params.session_id}/${params.page}`,
			);
	...
	const fetchAnswerData = async () => {
		try {
			const response = await axios.get(
				`/get_blank_answer/${params.session_id}/${params.page}`,
			);
	...
	const navigateToPage = (page: number) => {
		if (page >= 1 && questionData && page <= questionData.max_page) {
			router.push(
				`/t/course/${params.course_id}/preview/week/${params.week_id}/flow/${params.flow_id}/session/${params.session_id}/${page}`,
			);
		}
	};
	...
	const finishSession = async () => {
		try {
			...
			router.push(
				`/t/course/${params.course_id}/preview/week/${params.week_id}/flow/${params.flow_id}/completion/${params.session_id}`,
			);
		} catch (error) {
	...
	const renderQuestion = () => {
		...
		switch (questionData.question_type) {
			...
			case "MultipleTextQuestion": {
				const answerCount =
					questionData.question_content.match(/\[answer\d+\]/g)?.length || 1;
				return (
					<div className="space-y-4">
						{Array.from({ length: answerCount }, (_, i) => (
							<div key={i}>
								...
								<Input
									...
									onChange={(e) =>
										handleInputChange(`answer${i + 1}`, e.target.value)
									}
								/>
							</div>
						))}
								...
								<Checkbox
									...
									onCheckedChange={(checked) => {
										...
										if (checked) {
											...
											handleInputChange(
												"choices",
												currentChoices.filter((c: number) => c !== index),
											);
										}
									}}
								/>
