...
// ★ 変更点: EyeとRefreshCcwアイコンをインポート
import { ArrowLeft, ChevronLeft, ChevronRight, Eye, Move, RefreshCcw } from 'lucide-react';
...
import { MathJax, MathJaxSetup } from '@/components/shared/MathJax'; // MathJaxSetupをインポート
import TcAccessTime from '@/components/tc_access_time';
import { Button } from '@/components/ui/button';
// ★ 変更点: Card関連コンポーネントをインポート
...
function FlowPreviewPage() {
    ...
    // ... (saveSessionState, loadSessionState, clearSessionStateは変更なし)
    ...
    // ★ 変更点: プレビューリセット用の関数を追加
    const handleResetPreview = () => {
        clearSessionState();
        window.location.reload();
    };

    // ... (useEffectフックや他のハンドラ関数は変更なし)
    useEffect(() => {
    ...
    return (
        <MathJaxSetup> {/* ★ 変更点: MathJaxSetupで全体をラップ */}
            <TcAccessTime page="teacher_weekflows_preview_session" />
            <main>
                <div className="container mx-auto py-8">

                    {/* ★ 変更点: 教師用プレビューモードのバナーを追加 */}
                    <div className="bg-blue-100 border-l-4 border-blue-500 text-blue-800 p-4 mb-6 rounded-r-md flex justify-between items-center">
                        <div>
                            <p className="font-bold flex items-center gap-2">
                                <Eye className="w-5 h-5" />
                                教師用プレビューモード
                            </p>
                            <p className="text-sm mt-1">学生表示の確認、正解や解説の直接表示ができます。</p>
                        </div>
                        <Button variant="outline" size="sm" onClick={handleResetPreview} className="bg-white">
                    ...
                    <div className="mb-6">
                        <h1 className="text-2xl font-bold mb-2">{flowSession?.flow_title || '演習問題'}</h1>
                    ...
                    <div className="mb-8" onInput={resetHintTimer} onChange={resetHintTimer}>
                        ...
                        <Card className="mt-8 bg-gray-50 border-dashed">
                            <CardHeader>
                                <CardTitle className="text-base font-semibold text-gray-700">正解と解説（教師用）</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="border-t pt-4">
                                    {pageData?.answer_comment ? (
                                        <div className="space-y-2">
                                            <h4 className="font-bold text-sm text-gray-600">解説:</h4>
                                            <div className="p-3 border rounded bg-white text-sm">
                                                <MathJax text={pageData.answer_comment} />
                                            </div>
                                        </div>
                                    ) : <p className="text-sm text-gray-500">この問題には解説がありません。</p>}
                                </div>
                            </CardContent>
                        </Card>
