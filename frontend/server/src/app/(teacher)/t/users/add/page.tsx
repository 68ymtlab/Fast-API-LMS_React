...
import { AlertCircle, CheckCircle, UserPlus, Upload, Users } from "lucide-react";
...
function AddUserPage() {
  ...
  const validateUser = (userData: User, isFileUpload: boolean = false): string[] => {
    const errors: string[] = [];

    if (!userData.username) {
      errors.push(isFileUpload ? "ユーザー名が入力されていない箇所があります" : "ユーザー名を入力してください");
    }
    if (!userData.email) {
      errors.push(isFileUpload ? "メールアドレスが入力されていない箇所があります" : "メールアドレスを入力してください");
    }
    if (!userData.password) {
      errors.push(isFileUpload ? "パスワードが入力されていない箇所があります" : "パスワードを入力してください");
    }
    if (!userData.kind_name) {
      errors.push(isFileUpload ? "ユーザー種別が入力されていない箇所があります" : "ユーザー種別を選択してください");
    }
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setErrorMessages([response.data.error_msgs || "ユーザーの登録に失敗しました"]);
      }
    } catch (error: any) {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      for (const line of lines) {
        ...
        if (parts.length !== 4) {
          setErrorMessages(["CSVファイルのデータが間違っています。形式を確認してください"]);
          return;
        }
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setErrorMessages([response.data.error_msgs || "ユーザーの一括登録に失敗しました"]);
      }
    } catch (error: any) {
                      ...
                      <table className="w-full border border-gray-300 rounded-lg">
                        <thead>
                          <tr className="bg-gray-50">
                            ...
                            <th className="border border-gray-300 px-4 py-2 text-left">パスワード</th>
                            <th className="border border-gray-300 px-4 py-2 text-left">ユーザー種別</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr className="text-sm text-gray-600">
                            <td className="border border-gray-300 px-4 py-2">田中太郎</td>
                            ...
                            <td className="border border-gray-300 px-4 py-2">学生</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                      ...
                      <div className="max-h-40 overflow-y-auto border border-gray-300 rounded-lg">
                        <table className="w-full text-sm">
                          <thead className="bg-gray-50 sticky top-0">
                            <tr>
                              <th className="border-b border-gray-300 px-3 py-2 text-left">ユーザー名</th>
                              <th className="border-b border-gray-300 px-3 py-2 text-left">メール</th>
                              <th className="border-b border-gray-300 px-3 py-2 text-left">種別</th>
                            </tr>
                          </thead>
                          <tbody>
                            {fileUsers.map((user, index) => (
                              <tr key={index} className="hover:bg-gray-50">
                                <td className="border-b border-gray-200 px-3 py-2">{user.username}</td>
                                <td className="border-b border-gray-200 px-3 py-2">{user.email}</td>
                                <td className="border-b border-gray-200 px-3 py-2">{user.kind_name}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
