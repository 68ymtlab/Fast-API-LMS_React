      ...
      for (const subItem of obj) {
        if (typeof subItem === "object" && subItem !== null) {
          if ("flow_session_grade" in subItem) {
            ...
            if (s.flow_session_grade !== undefined && s.flow_session_grade > maxGrade) {
                ...
                for (const problem of Object.keys(exercise)) {
                  ...
                  if (Array.isArray(sessions)) {
                    for (const session of sessions) {
                      if (session?.finish_date_time && typeof session.flow_session_grade === "number") {
...
const Page = () => {
  ...
  const [lineData, setLineData] = useState<{ problem: string; date: Date; grade: string }[]>([]);
  ...
  useEffect(() => {
    const getStudentScore = async () => {
      try {
        const response = await axios.get<ScoreResponse>(`/get_flow_session_student_score/${course_id}`);
        if (response.data) {
