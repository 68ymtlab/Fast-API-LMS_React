...
import { ChevronDown, ChevronRight, Calendar, Target, AlertCircle } from "lucide-react";
...
const ProfilePage = () => {
  ...
  const processDate = (date: string) => date ? new Date(date).toLocaleString("ja-JP") : "未完了";
    ...
    <div className="min-h-screen bg-white p-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">コースの学習進捗と成績を確認できます</p>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-xl text-gray-800 flex items-center gap-3">
                      <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                        <span className="text-blue-600 font-semibold text-sm">{weekIdx + 1}</span>
                      </div>
                      {weekKey}
                    </CardTitle>
                    ...
                    {weekContent.length === 0 ? (
                      ...
                      <div className="space-y-4">
                        {weekContent.map((flowData, flowIdx) =>
                          Object.entries(flowData).map(([flowKey, flowContent]) => {
                            const flowKeyId = `${weekKeyId}-flow-${flowKey}`;
                            return (
                              <Card key={flowKeyId} className="border border-gray-200">
                                <CardHeader 
                                          ...
                                          <Card key={sessionKey} className={`border ${isCompleted ? 'border-green-200 bg-green-50' : 'border-gray-200'}`}>
                                            ...
                                            >
                                              <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex items-center gap-3">
                                                  <Badge variant={getGradeBadgeVariant(flowSession.flow_session_grade)}>
                                                    {Math.ceil(flowSession.flow_session_grade)}%
                                                  </Badge>
                                                  <Button variant="ghost" size="sm" className="p-2" disabled={isCompleted}>
                                                    {expanded[sessionKey] ? (
                                                      <ChevronDown className="h-4 w-4" />
                                                    ) : (
                                                      <ChevronRight className="h-4 w-4" />
                                              ...
                                              <div className="mt-3">
                                                <div className="flex items-center gap-2 mb-2">
                                                  ...
                                                  <span className={`text-sm font-medium ${getGradeColor(flowSession.flow_session_grade)}`}>
                                                    {Math.ceil(flowSession.flow_session_grade)}%
                                                  </span>
                                                </div>
                                                <Progress 
                                            ...
                                            {expanded[sessionKey] && !isCompleted && (
                                              <CardContent className="pt-0">
                                                <div className="bg-white rounded-lg border border-gray-200 p-4">
                                                  ...
                                                  {flowSession.flow_page.length === 0 ? (
                                                    <p className="text-gray-500 text-sm">間違えた問題はありません</p>
                                                  ) : (
                                                    <div className="space-y-3">
                                                      {flowSession.flow_page.map((flowPage, idx) => (
                                                        <Card key={idx} className="border border-orange-200 bg-orange-50">
                                                          <CardContent className="p-3">
                                                            <div className="flex items-start gap-2">
                                                              <span className="text-orange-600 font-medium text-sm">Q{idx + 1}</span>
