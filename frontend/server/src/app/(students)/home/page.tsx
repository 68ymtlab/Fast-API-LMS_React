...
import { User, Star, List, Crown, Trophy, Target, Book, Play, BarChart, FileText, Trash2, LogIn, BookOpen, Calendar, BarChart2, Clock, BookCheck } from "lucide-react";
  ...
  const fetchProgress = async () => {
    try {
      ...
      if (progresses.length > 0) {
        ...
        const average = parseFloat((total / progresses.length).toFixed(2));
        setProgress(average);
      } else {
        setProgress(0);
      }
    ...
    <>
      <main>
        <div className="flex flex-col items-start justify-start min-h-screen bg-gray-100 pt-20">
          <div className="container mx-auto px-8 py-8 max-w-7xl">
                ...
                <div className="bg-white rounded-lg shadow-md p-6 h-[210px]">
                  ...
                  <div className="space-y-2 h-[calc(200px-4rem)] overflow-y-auto pr-2">
                    {goals && goals.length > 0 ? (
                      goals.map((goal) => (
                        <div key={goal.goal_id} className="flex items-center justify-between p-3 bg-secondary/5 rounded-lg border border-secondary/10 hover:bg-secondary/10 transition-all duration-200">
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-medium text-lg">{subject.completedLessons}/{subject.totalLessons}</span>
                        </div>
            ...
            <Button onClick={logout} className="mt-4">ログアウト</Button>
      ...
      <Dialog open={goal_set_dialog} onOpenChange={set_goal_dialog}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick = {() => {set_goal_dialog(false)}}>
              キャンセル
            </Button>
          ...
          <div className="space-y-4 py-4">
            {completeGoals && completeGoals.length > 0 ? (
              completeGoals.map((goal) => (  
                <div key={goal.goal_id} className="flex items-center justify-between p-4 bg-secondary/5 rounded-lg border border-secondary/10 shadow-sm hover:shadow-md transition-all duration-200">
                  <div className="flex items-center gap-4">
                    <div className="p-2 bg-secondary/10 rounded-full">
