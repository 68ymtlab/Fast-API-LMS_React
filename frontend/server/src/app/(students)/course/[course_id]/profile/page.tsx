	...
	const fetchFlowSessionHistory = async () => {
		try {
			...
			const response = await axios.get(
				`/get_flow_session_history/${params.course_id}`,
			);
								...
								<div className="space-y-4">
									{flowSessionHistory.map((week, weekIndex) => {
										...
										if (!week || typeof week !== "object") {
											console.warn(
												`Week at index ${weekIndex} is not an object:`,
												week,
											);
											return null;
										}
											...
											<div key={Object.keys(week)[0] || weekIndex}>
												{Object.entries(week).map(([weekKey, weekContent]) => {
													...
													if (!weekContent || typeof weekContent !== "object") {
														console.warn(
															`WeekContent for ${weekKey} is not an object:`,
															weekContent,
														);
														return null;
													}
													...
													return (
														<Card key={weekKey} className="overflow-hidden">
															<Collapsible
																open={isWeekExpanded}
																onOpenChange={() =>
																	toggleWeekExpansion(weekIndex, weekKey)
																}
															>
																...
																<CollapsibleContent>
																	<CardContent className="pt-0">
																		<div className="space-y-3">
																			{Object.entries(weekContent).map(
																				([flowKey, flowContent], flowIndex) => {
																					console.log(
																						`FlowContent for ${flowKey}:`,
																						flowContent,
																					);

																					const flowExpandKey = `${weekIndex}-${weekKey}-${flowIndex}-${flowKey}`;
																					const isFlowExpanded =
																						expandedFlows[flowExpandKey];

																					// flowContentが配列でない場合のエラーハンドリング
																					if (!Array.isArray(flowContent)) {
																						console.warn(
																							`flowContent is not an array for ${flowKey}:`,
																							flowContent,
																						);
																						return null;
																					}
