...
function FlowSessionPage() {
  ...
  useEffect(() => {
    const checkAllAnswerStatus = async () => {
      ...
      for (let page = 1; page <= flowSession.total_pages; page++) {
        try {
          const answerResponse = await axios.get(`/get_blank_answer/${flowSession.flow_session_id}/${page}`);
  ...
  useEffect(() => {
    const fetchPageData = async () => {
      ...
      try {
        ...
        // Get page content
        const pageResponse = await axios.get(`/get_flowpage/${flowSession.flow_session_id}/${currentPage}`);

        // Get blank answers if exists
        const answerResponse = await axios.get(`/get_blank_answer/${flowSession.flow_session_id}/${currentPage}`);
  ...
  return (
    <main>
      <div className="container mx-auto py-8 sm:px-4 p-4 md:px-8 py-4 sm:py-8 max-w-full md:max-w-7xl">
        {/* Header */}
        <div className="mb-6">
          ...
          {Array.from({ length: flowSession?.total_pages || 1 }, (_, i) => {
            ...
            return (
              <button
                ...
                `}
                title={`問題 ${pageNum} (${
                  status === "correct" ? "正解" : status === "incorrect" ? "不正解" : "未解答"
                })`}
              >
                {pageNum}
              </button>
